...
import { GET_TOTAL_TIME, PLAN_ADD, PLAN_CHECK, PLAN_GET, PLAN_SELECT_DAY, STAT_SELECT_DAY } from './types';
...
export function getTimeInfo(dataTosubmit) {
  return (dispatch) => {
    axios.post('/api/get-stat', dataTosubmit).then(({ data: { success, message, dailyTotalTimes } }) => {
...
export function statSelectDay(dataTosubmit) {
  return (dispatch) => {
    axios.post('/api/get-dailyLapse', dataTosubmit).then(({ data: { success, message, dailyLapses } }) => {
      if (!success) return dispatch(catchError(message));
